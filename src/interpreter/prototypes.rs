...
use crate::{interpreter::{RuntimeError, RuntimeResult}, parser::{Expression, ValueHolder}};
...
pub type MethodFunc = Arc<dyn Fn(&ValueHolder, Vec<Expression>) -> RuntimeResult + Send + Sync>;
...
impl Prototype {
    ...
    pub fn operate(&self, operation: Operation, a: &ValueHolder, b: &ValueHolder) -> RuntimeResult {
        let func = self.operators.get(&operation).ok_or(RuntimeError::OperationNotSupported(format!("{:?}", operation)))?;

        func(a, b)
    }

    pub fn get_method(&self, name: &str) -> Option<&Arc<dyn Fn(&ValueHolder, Vec<Expression>) -> RuntimeResult + Send + Sync>> {
...
lazy_static! {
    pub static ref OBJECT_PROTOTYPE: Prototype = {
        ...
        prototype
            .with_method("toString", Arc::new(|instance, args| {
                if args.len() > 0 {
                    return Err(RuntimeError::Custom("Too many arguments".into()));
                }
        ...
        prototype
            .with_operator(Operation::Addition, Box::new(|a, b| {
                let ValueHolder::String(value) = a else {
                    return Err(super::RuntimeError::InvalidType("Expected string".to_string()))
                };
            ...
            .with_method("upper", Arc::new(|instance, args| {
            ...
            .with_method("lower", Arc::new(|instance, args| {
                let ValueHolder::String(string) = instance else {
                    unreachable!()
                };
        ...
        prototype
            .with_operator(Operation::Addition, Box::new(|a, b| {
                let ValueHolder::Int(a) = a else {
                    return Err(super::RuntimeError::InvalidType("Expected int".to_string()))
                };

                let b = match b {
                    ValueHolder::Int(value) => value,
                    ValueHolder::Float(value) => return Ok(ValueHolder::Float(*a as f64 + *value)),
                    _ => return Err(super::RuntimeError::InvalidType("Expected number".to_string()))
                };
        ...
        prototype
            .with_operator(Operation::Addition, Box::new(|a, b| {
                let ValueHolder::Float(a) = a else {
                    return Err(super::RuntimeError::InvalidType("Expected float".to_string()))
                };
